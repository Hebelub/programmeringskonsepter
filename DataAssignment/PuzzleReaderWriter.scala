...
object PuzzleReaderWriter{
  ...
  def getPuzzle(index:Int):Puzzle={
	...
	return new Puzzle(puzzle.getSizex(),puzzle.getSizey(),"")
  }

  def putSolution(puzzle:Puzzle)={
    val puzzleOut=Schema.Solution.newBuilder().setSizex(puzzle.sizeX).setSizey(puzzle.sizeY).setSol(puzzle.solution).build();
